<?php
/**
  * @var \App\View\AppView $this
  */
?>
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Html->link(__('Edit Funcionario'), ['action' => 'edit', $funcionario->siape]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Funcionario'), ['action' => 'delete', $funcionario->siape], ['confirm' => __('Are you sure you want to delete # {0}?', $funcionario->siape)]) ?> </li>
        <li><?= $this->Html->link(__('List Funcionarios'), ['action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Funcionario'), ['action' => 'add']) ?> </li>
        <li><?= $this->Html->link(__('List Pessoas'), ['controller' => 'Pessoas', 'action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Pessoa'), ['controller' => 'Pessoas', 'action' => 'add']) ?> </li>
    </ul>
</nav>
<div class="funcionarios view large-9 medium-8 columns content">
    <h3><?= h($funcionario->siape) ?></h3>
    <table class="vertical-table">
        <tr>
            <th scope="row"><?= __('Siape') ?></th>
            <td><?= h($funcionario->siape) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Funcao') ?></th>
            <td><?= h($funcionario->funcao) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Setor') ?></th>
            <td><?= h($funcionario->setor) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Email') ?></th>
            <td><?= h($funcionario->email) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Endereco') ?></th>
            <td><?= h($funcionario->endereco) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Contato') ?></th>
            <td><?= h($funcionario->contato) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Pessoa') ?></th>
            <td><?= $funcionario->has('pessoa') ? $this->Html->link($funcionario->pessoa->id, ['controller' => 'Pessoas', 'action' => 'view', $funcionario->pessoa->id]) : '' ?></td>
        </tr>
    </table>
</div>
